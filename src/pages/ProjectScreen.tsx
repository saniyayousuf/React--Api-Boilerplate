import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { Delete, Get } from "../config/API-METHODS/api_methods";



export default function Pro_Card() {


    const [listData, setListData] = useState<any>([]);

    const navigate = useNavigate();

    const deletePost = (id: any) => {
        Delete(`posts/${id}`)
            .then(() => {
                console.log("Post Deleted Successfully");
            })
            .catch((err) => {
                console.log(err);
            });
    };

    let getData = () => {
        Get("posts")
            .then((res) => {
                setListData([...res.data]);
            })
            .catch((err) => {
                console.log(err.message);
            });
    };

    useEffect(() => {
        getData();
    }, []);

    return <>
        <div className="bg-yellow-100">
            <div className=" text-center py-3">
                <h1 className="font-serif">Share Your Thoughts</h1>

                <button
                    onClick={() => {
                        navigate("/add");
                    }}
                    style={{
                        // backgroundColor: 'blue',    
                        // color: 'white',            
                        display: 'block',
                        margin: '0px auto',
                        padding: '10px',
                        marginTop: '10px'
                    }}
                    className="font-serif  bg-blue-500 hover:bg-blue-700 border-x-4 border-amber-100 text-white font-bold  rounded "
                >
                    Comment Now
                </button>
            </div>

            {listData &&
                Array.isArray(listData) &&
                listData.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 px-3  ">
                        {listData.map((item, index) => (
                            <div
                                key={index}
                                className="max-w-md rounded overflow-hidden shadow-md rounded-br-lg  border-x-4 border-indigo-400 ...   bg-gradient-to-r from-green-200 to-blue-300 hover:from-pink-200 hover:to-yellow-300 p-2 my-2  bg-blend-lighten "
                            >
                                <div className="md:flex">

                                    <div className="md:w-6/7 p-4">

                                        <div>
                                            <h2 className="text-gray-900 text-xl mt-2 font-semibold border-double border-4 border-indigo-300  inline-flex  m-2 ms-5 p-1 px-3">
                                                UserID :  {item.id}
                                            </h2>
                                        </div>
                                        <div className="font-bold text-xl mb-2">Title :   {item.title}</div>
                                        <p className="text-gray-800 text-base">
                                            <b> Description :</b>  "{item.body}"
                                        </p>

                                        <div className="mt-4 ">
                                            <button
                                                onClick={() => deletePost(item.id)}
                                                className="bg-blue-500 hover:bg-blue-700 border-x-4 border-amber-100 text-white font-bold py-2 px-4  me-5 rounded ">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-6 h-6">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                                </svg>

                                            </button>
                                            <button
                                                onClick={() => { navigate(`/add/${item.id}`) }}
                                                className="bg-blue-500 hover:bg-blue-700 border-x-4 border-amber-100 text-white font-bold py-2 px-4 rounded">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-6 h-6">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                                </svg>

                                            </button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ))}
                    </div>
                )}
        </div>


    </>
}


